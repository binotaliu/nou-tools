...
- **Seats are a roving-tabindex grid** (`useSeatRovingFocus`): one Tab stop per floor, Left/Right in DOM order, Up/Down by on-screen position, Home/End. Every seat button (and every list-view row header) carries `data-seat-code`. **Occupied seats, and every seat once you hold one, are `aria-disabled`, never `disabled`**, since a disabled button drops focus and hides its occupant; `grid.activateSeat()` does the guarding. Seat names come from `seatAriaLabel(seat, timer.spokenTimerLabel(seat))`, rounded to minutes so they don't change every second.

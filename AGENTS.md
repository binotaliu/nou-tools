...
- **Form dates use `Components/DateField.vue`, not `<input type="date">`.** Native date inputs render dd/mm/yyyy or mm/dd/yyyy per browser and Safari shows today's date for an empty value. `DateField` shows a fixed `M/D` label (or 未設定), submits `YYYY-MM-DD` through a hidden input (so it works in native form POSTs like the learning-progress form) and teleports its calendar to `<body>` so `overflow-x-auto` tables can't clip it. Pass the server's `now` as `today` so it follows Taipei time. (The homepage's 今日視訊面授 date filter still uses a native input.)

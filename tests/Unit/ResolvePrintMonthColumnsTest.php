 ...
 */
function printMonth(int $classDays, array $courseNames): SchedulePrintMonthViewModel
{
...
it('keeps three columns for an ordinary schedule', function () {
    $months = array_fill(0, 5, printMonth(6, ['心理學', '會計學']));

    expect((new ResolvePrintMonthColumns)($months))->toBe(3);
});

it('switches to four columns when long course lists would push the calendars off the page', function () {
...
it('keeps three columns when four would not be any shorter', function () {
...
it('always uses four columns from seven months on', function () {

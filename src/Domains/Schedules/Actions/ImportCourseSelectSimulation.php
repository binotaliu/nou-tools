...
final readonly class ImportCourseSelectSimulation
{
     ...
     * other class. Idempotent: re-running updates existing tentative
     * classes and drops dates no longer present in the source data.
     *
    ...
    private function importTerm(array $data, string $term): int
    {
        ...
        foreach ($videoSessions as $simCourseId => $sessions) {
            ...
            $course = $this->findCourse($term, $name);
            ...
            foreach ($sessions as $label => $session) {
                ...
                if (! $type instanceof CourseClassType || ! is_array($session)) {
                    continue;
                }
     ...
     */
    private function upsertTentativeClass(Course $course, CourseClassType $type, string $start, string $end, array $dates): void
    {
        $courseClass = CourseClass::query()->updateOrCreate(
            ...
            [
                'code' => 'NOTICE-'.strtoupper($type->value),
                'start_time' => $start,
                'end_time' => $end,
                'teacher_name' => '',
                'link' => '',
    ...
    private function findCourse(string $term, string $name): ?Course
    {
